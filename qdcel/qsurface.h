 ...
 * represents  a triangle  surface mesh  (i.e., a  quadrilateral surface)
 * with empty boundary.
 *
 * \author
...
#include "qhalfedge.h"  // QHalfedge
#include "qedge.h"      // QEdge
#include "qface.h"      // QFace
...
/**
   * \class QSurface
   *
   * \brief This  class represents a quadrilateral surface  mesh with empty
   * boundary.
   */
template <
...
public:
     ...
     * \typedef QVertex
     *
     * \brief Defines  QVertex as  an alias for  qdcel::QVertex< VAttrib,
     ...
     * \typedef QFace
     *
     * \brief  Defines  QFace  as  an alias  for  qdcel::QFace<  VAttrib,
     * FAttrib , EAttrib , HAttrib >.
     */    
     ...
     * \typedef QEdge
     *
     * \brief  Defines  QEdge  as  an alias  for  qdcel::QEdge<  VAttrib,
     * FAttrib , EAttrib , HAttrib >.
     */    
    ...
    /**
     * \typedef QVertexIterator
     *
     * \brief Defines  an iterator  for the list  of vertices  of this
     * mesh.
     */    
    typedef typename std::list< QVertex* >::const_iterator QVertexIterator ;


    /** 
     * \typedef QEdgeIterator
     *
     * \brief Defines an iterator for the list of edge of this mesh.
     */    
    typedef typename std::list< QEdge* >::const_iterator QEdgeIterator ;


    /** 
     * \typedef QFaceIterator
     *
     * \brief Defines an iterator for the list of faces of this mesh.
     */    
    typedef typename std::list< QFace* >::const_iterator QFaceIterator ;
     ...
     * pointers.
     */
    typedef typename std::map< unsigned , QVertex* > VMAP ;
    ...
    /**
     * \fn QSurface( unsigned nverts , double* lverts , unsigned nfaces , unsigned* lfaces )
     *
    ...
    /**
     * \fn inline QVertexIterator vertices_begin() const
     *
     ...
     * list.
     */
    inline QVertexIterator vertices_begin() const
    { 
    ...
    /**
     * \fn inline QEdgeIterator edges_begin() const
     *
     ...
     * list.
     */
    inline QEdgeIterator edges_begin() const
    {
    ...
    /**
     * \fn QFaceIterator faces_begin() const
     *
     ...
     */
    QFaceIterator faces_begin() const
    {
    ...
    /**
     * \fn inline QVertexIterator vertices_end() const
     *
     ...
     * list.
     */
    inline QVertexIterator vertices_end() const
    { 
    ...
    /**
     * \fn inline QEdgeIterator edges_end() const
     *
     ...
     * list.
     */
    inline QEdgeIterator edges_end() const
    {
    ...
    /**
     * \fn QFaceIterator faces_end() const
     *
...
protected:
    ...
    std::list< QVertex* > _list_of_vertices ;  ///< List of vertices of this surface.


    std::list< QEdge* > _list_of_edges ;  ///< List of edges of this surface.


    std::list< QFace* > _list_of_faces ;  ///< List of faces of this surface.

} ;


/**
   * \fn QSurface< VAttrib , FAttrib , EAttrib , HAttrib >::QSurface( unsigned nverts , double* lverts , unsigned nfaces , unsigned* lfaces )
   *
   * \brief Creates an instance of this class.
   *
   * \param nverts The number of vertices of this surface.
...
template <
        ...
        typename HAttrib
        >
QSurface< VAttrib , FAttrib , EAttrib , HAttrib >::QSurface(
    unsigned nverts ,
    double* lverts ,
    unsigned nfaces ,
    unsigned* lfaces
...
/**
   * \fn QSurface< VAttrib , FAttrib , EAttrib , HAttrib >::~QSurface()
   *
...
QSurface< VAttrib , FAttrib , EAttrib , HAttrib >::~QSurface()
{
     ...
     */
    for( QVertexIterator vit = vertices_begin() ; vit != vertices_end() ; ++vit ) {
        QVertex* vertex = *vit ;
     ...
     */
    for( QEdgeIterator eit = edges_begin() ; eit != edges_end() ; ++eit ) {
        QEdge* edge = *eit ;

        QHalfedge* h1 = edge->get_first_halfedge() ;
        QHalfedge* h2 = edge->get_second_halfedge() ;
     ...
     */
    for( QFaceIterator fit = faces_begin() ; fit != faces_end() ; ++fit ) {
        QFace* face = *fit ;
...
/**
   * \fn void \fn QSurface< VAttrib , FAttrib , EAttrib , HAttrib >::create_vertices( unsigned nverts , double* lverts , VMAP& vmap )
   *
   * \brief Creates the vertices of this surface.
   *
   * \param nverts The number of vertices of this surface.
...
template <
        ...
        >
void
QSurface< VAttrib , FAttrib , EAttrib , HAttrib >::create_vertices(
    unsigned nverts ,
    double* lverts ,
    VMAP& vmap
    )
...
/**
   * \fn void \fn QSurface< VAttrib , FAttrib , EAttrib , HAttrib >::create_faces( unsigned nfaces , unsigned* lfaces , HMAP& hmap , VMAP& vmap )
   * 
   * \brief Creates the faces and half-edges of this surface.
   *
   * \param nfaces The number of faces of this surface.
...
template <
        ...
        >
void
QSurface< VAttrib , FAttrib , EAttrib , HAttrib >::create_faces(
    unsigned nfaces ,
    unsigned* lfaces ,
    HMAP& hmap ,
    VMAP& vmap
    )
{
     ...
     * iteration i, create  the i-th face ( i.e.,  a quad) of this
     * surface.
     */
    for ( unsigned i = 0 ; i < nfaces ; i++ ) {
        ...
        QFace* face = new QFace( 0 ) ;

        //
        // Create the four half-edges of the i-th face.
        //
       ...
       */
        for ( unsigned k = 0 ; k < 4 ; k++ ) {
            ...
            const unsigned v2 = lfaces[ j + ( ( k + 1 ) % 4 ) ] ;
...
/**
   * \fn void QSurface< VAttrib , FAttrib , EAttrib , HAttrib >::create_edges( HMAP& hmap )
   *
   * \brief Creates the edges of this surface.
   *
   * \param hmap A hash table with half-edge identifiers and pointers.
...
template <
        ...
        >
void
QSurface< VAttrib , FAttrib , EAttrib , HAttrib >::create_edges(
    HMAP& hmap
    )
{
    /**
    ...
    for ( typename HMAP::iterator hit = hmap.begin(); 
       ...
       */
         ...
         * half-edge.
         */
            QEdge* edge = new QEdge( hit->second , 0 ) ;
...
/**
   * \fn bool \fn QSurface< VAttrib , FAttrib , EAttrib , HAttrib >::is_consistent() const
   *
...
template <
        ...
        >
bool
QSurface< VAttrib , FAttrib , EAttrib , HAttrib >::is_consistent() const
{
    //
    ...
    for ( QFaceIterator fit = faces_begin(); fit != faces_end() ; ++fit ) {
       ...
       */
        ...
        /**
       * Get the other three half-edges of the face.
       */
        QHalfedge* h2 = h1->get_next() ;
        QHalfedge* h3 = h2->get_next() ;
        QHalfedge* h4 = h3->get_next() ;
       ...
       */
        if ( ( h1->get_face() != h2->get_face() ) ||
                ...
                ( h1->get_face() != h4->get_face() ) ||
                ( h2->get_face() != h3->get_face() ) ||
                ( h2->get_face() != h4->get_face() ) ||
                ( h3->get_face() != h4->get_face() )
                )
        {
            return false ;
        }
       ...
       */
        if ( h1->get_next() != h2 ) return false ;
        if ( h1->get_prev() != h4 ) return false ;

        if ( h2->get_next() != h3 ) return false ;
        if ( h2->get_prev() != h1 ) return false ;

        if ( h3->get_next() != h4 ) return false ;
        if ( h3->get_prev() != h2 ) return false ;

        if ( h4->get_next() != h1 ) return false ;
        if ( h4->get_prev() != h3 ) return false ;
       ...
       */
        ...
        if ( h4->get_origin() == 0 ) return false ;
       ...
       */
        ...
        if (
                ( h4 != h4->get_edge()->get_first_halfedge()  ) &&
                ( h4 != h4->get_edge()->get_second_halfedge() )
                )
        {
            return false ;
        }
       ...
       */
        ...
        if ( h4->get_mate()->get_mate() != h4 ) return false ;
    ...
    for ( QVertexIterator vit = vertices_begin() ; vit != vertices_end() ; ++vit ) {
       ...
       */
        ...
        QHalfedge* h2 = h1->get_prev()->get_mate() ;
        do {
            /**
  * The half-edge pointer to  the origin vertex must agree with
  * the pointer to the current vertex.
  */
    ...
    for( QEdgeIterator eit = edges_begin(); eit != edges_end() ; ++eit ) {
       ...
       */ 
        QHalfedge* h1 = (*eit)->get_first_halfedge() ;
       ...
       */ 
        QHalfedge* h2 = (*eit)->get_second_halfedge() ;
